...
| 2026-07-22 | F4 | 100件の翻訳後、slop_gateが9件を誤検出(「見てみましょう」「見ていきましょう」) | 「見てみましょう」「見ていきましょう」は勧誘型のhype語のつもりでslop_wordsに入れたが、実際は日本語の説明文で普通に使われる中立的な言い回しで、F2のKorean「다음은」と同種の誤検出だった | 単独の「見てみましょう」「見ていきましょう」をslop_wordsから除去、より明確にAI口調な「一緒に見ていきましょう」(皆さんへの呼びかけを伴う形)のみ維持 | slop_gate.sh 補正 |
